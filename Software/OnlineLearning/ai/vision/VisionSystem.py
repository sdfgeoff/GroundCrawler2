...
from ai.vision.image_util import get_filtered_scaled, image_to_tensor, tensor_to_image, tile_images, get_filtered_patch, NumpyImage
...
class VisionModel:
    def __init__(self, config: video_config.VideoModelConfig):
        ...
        if config.initial_model_path:
            self.model.load_state_dict(torch.load(  # type: ignore
    ...
    def add_training_frame(self, frame: NumpyImage):
        if self.training_setup is None:
            initial_memory_images = [image_to_tensor(get_filtered_patch(frame, self.config.resolution), self.config.device) for _ in range(self.config.cache_size)]
            ...
            self.training_setup = TrainingSetup(
                ...
                training_data=replace_random_image(self.training_setup.training_data, get_filtered_patch(frame, self.config.resolution)),
            )
    ...
    def do_training_pass(self):
        ...
        for batch, images in enumerate(self.training_setup.training_data.dataloader):
            if self.training_iteration % (self.config.cache_size / self.config.batch_size) != batch:
                continue
        ...
        if self.training_iteration % 20000 == 0:
            torch.save(self.model.state_dict(), f"video_model-{self.training_iteration}.raw")  # type: ignore
    ...
    def encode_decode_frames(self, frames: list[NumpyImage]) -> list[NumpyImage]:
        with torch.no_grad():
            ...
            images_to_process = torch.stack(tuple(
                image_to_tensor(f, device=self.config.device) for f in frames
            ))
...
class VisionSystem:
    ...
    def ingest_frame(self, new_image_raw: NumpyImage):
        ...
        if self.config.train_enabled:
            if self.frame_counter % self.config.training_dataset_frames_between_update == 0:
                ...
                if self.config.training_dataset_video_save:
                    ...
                    fancy_log.save_frame(
                        ...
                        tile_images([tensor_to_image(d) for d, _f in self.vision_model.training_setup.training_data.dataset]).astype(numpy.uint8)
                    )
        ...
        new_image_scaled = get_filtered_scaled(new_image_raw, self.config.vision_model.resolution)
